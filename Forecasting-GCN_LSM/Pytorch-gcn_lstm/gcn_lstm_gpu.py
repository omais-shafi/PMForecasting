...
class GCN_Layer(nn.Module):
    def __init__(self, grid_size, input_dim, output_dim, adj):
        ...
        self.kernel=torch.nn.Parameter(torch.randn(self.d3, self.d4,device=device))
        # self.kernel = self.kernel.to(device)
        self.bias=torch.nn.Parameter(torch.randn(self.d2, 1,device=device))
...
class GCN_LSTM(nn.Module):
    def __init__(self, A, grid_size, seq_len, gc_sizes, lstm_sizes):
        ...
        adj_init = torch.zeros(grid_size, grid_size,device=device)
        ...
        for i in range(len(self.lstm_sizes)):
            if i==0:
                self.lstm_layers = [nn.LSTM(input_size=grid_size, hidden_size=self.lstm_sizes[i], batch_first=True,device=device)]
            else:
                self.lstm_layers.append(nn.LSTM(input_size=self.lstm_sizes[i-1], hidden_size=self.lstm_sizes[i], batch_first=True,device=device))
        ##MANUAL
